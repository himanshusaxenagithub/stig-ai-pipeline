...
A **[worked example](https://stig.hsaxena.com/demo.html)** (`docs/demo.html`) shows the same path as an article — pick OS, select CAT I, download, approve, read the risk story — with labeled sample data, so a new reader does not have to click through a real scan. **[Ten short essays](https://stig.hsaxena.com/articles/)** (`docs/articles/`) explain the public-interest case in plain English: what a STIG is, why a school or clinic without a security team might care, why a person still types their name, and why a report that hides skipped rules is not useful.
